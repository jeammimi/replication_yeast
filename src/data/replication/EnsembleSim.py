...
class ensembleSim:
    ...
    def __init__(self, Nsim, Nori, Ndiff, lengths, p_on, p_off, only_one, all_same_ori=False, l_ori=[], cut=10):
        self.Nsim = Nsim
        self.Nori = Nori
        self.Ndiff = Ndiff
    ...
    def run_all(self, run_length=200):
        ...
        for sim in range(self.Nsim):
            ...
            for poly in S.polys:
                ...
                self.aDNAs[-1].append(poly.get_DNA_with_time())
            ...
            self.aDNAs[-1] = 1 + np.sum(np.array(self.aDNAs[-1]), axis=0) / np.sum(self.lengths)
    ...
    def get_quant(self, name, shift=0):
        ...
        normed_prop = np.zeros((self.Nsim, maxl)) + shift
