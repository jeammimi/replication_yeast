...
def create_init_conf_yeast(len_chrom=len_chrom,p_ribo=p_ribo,dist_centro=dist_centro,Radius=Radius,Mt=Mt,dnuc=dnuc):
    ...
    for X in range(Nchromosomes):
        ...
        S_centered =  Spherical(V(0,0,0),radius=Radius-Mt*0.85) # a sphere centered that intersect the spb sphere
        ...
        if X != 11:
            ...
            p_r,size = p_ribo[11]
            Beads = [2]+[1]*(d1-2)+[3]+[1]*(p_r-d1)+[4]*size + [1]*(len_chrom[X]-p_r - 1) + [2]
            #print(Beads)
            ...
            Sim.add(Polymer(N=len_chrom[X] + size,type_bead=Beads,
                            ...
                            lconstrain=[Point(index=0,position=telo1._v),
                                        Point(index=d1,position=centromere._v),
                                        Point(index=int(p_r+size/2),position=(0.66*Radius,0,0)), #We add a new constrain: the center 
